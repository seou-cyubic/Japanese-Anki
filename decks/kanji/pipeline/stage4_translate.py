...
import paths
from meanings import (AGREE_SLOT, CLAUDE_REVIEWER, DRAFT_SLOT, MAX_SENSES,
                      REVIEW_MODEL, REVIEW_SLOT, agree_key, claude_key, clean,
                      final_ko, load_claude_review, pro_ko, review_key)
import re
import json
import time
...
def ask_json(client, prompt):
    """호출 하나가 깨져도 전체를 세우지 않는다 — 그 묶음은 빈 답으로 돌려준다.

    모델이 가끔 JSON 을 반쯤 깨뜨려 내고, 429 가 재시도를 다 써 버리기도 한다.  예전에는
    그 한 건이 스레드 풀째 스테이지를 세웠고, 이미 받았지만 아직 캐시에 쓰지 않은 다른
    묶음의 답까지 잃었다.  빈 답을 받은 항목은 캐시에 들어가지 않으므로 다음 실행이 다시 묻는다.
    """
    try:
        answer = client.json(prompt)
    except Exception as error:          # noqa: BLE001 — 다음 실행이 다시 묻는다
        print(f"  !! 묶음 하나를 건너뛴다: {type(error).__name__}: {str(error)[:120]}", flush=True)
        return {}
    if isinstance(answer, dict):
        return answer
    return {"results": answer if isinstance(answer, list) else []}


BATCH = 40                 # 한 번에 묻는 용례 수.  항목마다 sino 를 먼저 따지게 해 무거워졌다
WORKERS = 8                # 동시에 띄우는 호출 수

# **최우선 규칙은 세 프롬프트(초안·감수·맞추기)가 글자 그대로 나눠 쓴다.**  단계마다
# 따로 적으면 한쪽만 고쳐지고, 감수가 초안과 다른 잣대로 보게 된다.
#
# 이 블록이 따로 선 이유: ``v3`` 의 프롬프트에도 같은 뜻을 두 원소로 적지 말라(5-1),
# 한자음 그대로를 뜻으로 쓰지 말라(5-2) 가 있었다.  그런데도 ``四日 -> 나흘 / 4일``,
# ``雨量 -> 우량``, ``再三 -> 재삼`` 이 초안과 pro 감수를 모두 통과해 카드에 실렸다.
# 규칙이 아홉 개 가운데 하나로 묻혀 있었고, 판정 절차 없이 '하지 말라' 만 있었다.
# 그래서 맨 앞에 두고, 묻는 질문과 검사를 절차로 적고, 출력에 ``sino`` 칸을 두어 한자음을
# **먼저 적은 뒤** 판정하게 한다.
#
# 이 문자열에는 ``%`` 를 쓰지 않는다 — 프롬프트 전체가 ``%`` 서식으로 채워진다.
CORE_RULES = """━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[최우선 규칙 — 이 작업의 품질은 이 두 규칙으로 결정된다]
아래 A·B 는 다른 모든 규칙보다 앞선다. 다른 규칙을 전부 지켜도 A·B 중 하나라도 어긴
항목이 있으면 그 답은 쓸 수 없는 답이다. 지금까지 가장 많이 틀린 곳이 바로 여기다.
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

■ 규칙 A — 뜻 하나에는 한국어 **한 마디만** 적는다

ko 배열의 원소 하나는 **서로 다른 뜻 하나**다. 같은 뜻을 다르게 말한 표현을 늘어놓는
자리가 아니다.

원소를 두 개 이상 적었다면, **모든 쌍**에 대해 반드시 이렇게 묻는다.
  "이 둘 중 하나를 지우면, 학습자가 알 수 없게 되는 **뜻**이 생기는가?"
  → 생기지 않는다 = 같은 뜻이다. 대표 하나만 남기고 나머지는 **지운다.**
  → 생긴다       = 다른 뜻이다. 그때만 둘 다 남긴다.

같은 뜻을 두 번 적는 전형 — 아래 꼴은 덱 전체에서 **한 건도 나와서는 안 된다.**
  고유어와 숫자 표기     四日   ["나흘", "4일"]            → ["나흘"]
                        三日   ["사흘", "3일"]            → ["사흘"]
                        二十歳 ["스무 살", "20세"]        → ["스무 살"]
  고유어와 한자어        器     ["그릇", "용기"]           → ["그릇"]
                        育つ   ["자라다", "성장하다"]      → ["자라다"]
                        右     ["오른쪽", "우측"]         → ["오른쪽"]
  같은 뜻의 두 한자어    女性   ["여성", "여자"]           → ["여성"]
                        決意   ["결의", "결심"]           → ["결심"]
  낱말과 그 풀이         意味   ["의미", "뜻"]             → ["의미"]
  같은 뜻의 두 용언      汚す   ["더럽히다", "오염시키다"]  → ["더럽히다"]
뜻이 정말 다를 때만 나눈다.
                        買う   ["사다", "높이 평가하다"]   → 둘 다 남긴다
                        目玉   ["눈알", "주요 상품"]       → 둘 다 남긴다

■ 규칙 B — 그 한 마디는 **한국 사람이 그 뜻으로 실제로 쓰는 가장 흔한 말**이어야 한다

일본어 한자어를 한국 한자음으로 읽으면 한국어처럼 **보인다.** 그러나 그렇게 읽은 말이
한국어에서는 쓰이지 않거나, 드물게만 쓰이거나, 다른 뜻으로 먼저 읽히는 일이 매우 많다.
그래서 w 를 한국 한자음으로 읽은 말(sino)을 뜻으로 쓰려면 **아래 네 검사를 모두
통과해야 한다.** 하나라도 걸리면 그 말을 버리고, 한국 사람이 그 뜻으로 실제로 쓰는
말로 다시 적는다. sino 의 일부만 바꾼 말(글자를 더하거나 뺀 말)도 똑같이 검사한다.

  검사 1 — 흔한가         오늘날 한국어 일상 대화·뉴스·교과서에 **그 뜻으로** 흔히 나오는 말인가?
                          사전 구석에만 있는 말, 문어투·한문투·옛말은 탈락이다.
  검사 2 — 먼저 떠오르는가  한국 사람이 그 말을 듣자마자 **이 뜻을 먼저** 떠올리는가?
                          같은 소리의 다른 말이 더 흔하면 탈락이다.
  검사 3 — 표준 꼴인가     한국어에서는 글자를 더하거나 바꾼 **다른 꼴**이 표준이지 않은가?
                          그렇다면 그 표준 꼴을 쓴다.
  검사 4 — 내가 쓰겠는가   한국 사람에게 이 낱말을 설명하면서 **내 입으로 그 말을 하겠는가?**

탈락해서 고친 예 — 이런 답은 덱 전체에서 **한 건도 남아서는 안 된다.**
  雨量   "우량"    → "강우량"         검사 2·3  '우량' 은 '우량 기업' 의 優良이 먼저 떠오르고, 표준은 '강우량'
  再三   "재삼"    → "여러 번"        검사 1·4  문어투라 일상에서 거의 쓰지 않는다
  休止   "휴지"    → "멈춤"           검사 2    '휴지' 는 화장실 휴지가 먼저다
  悪事   "악사"    → "나쁜 짓"        검사 1·2  '악사' 는 樂士가 먼저다
  畏怖   "외포"    → "두려워함"       검사 1    한국어에서 쓰지 않는 말이다
  光陰   "광음"    → "세월"           검사 1
  胃弱   "위약"    → "위가 약함"      검사 1·2
  疫病神 "역병신"  → "불행을 몰고 오는 사람"  검사 1
통과해서 그대로 두는 예
  人口 "인구" · 愛情 "애정" · 握手 "악수" · 会議 "회의" · 学校 "학교"
  — 한국어에서도 바로 그 뜻으로 매일 쓰는 말이다.

판단이 갈리면 **한자음 쪽을 버리고 한국 사람이 더 흔히 쓰는 말을 고른다.**
한자음과 풀어 쓴 말을 둘 다 원소로 적어 양쪽을 살리는 것은 규칙 A 위반이다.
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

"""

NOTATION_RULES = """[적는 형식]
- 뜻이 여럿이면 ko 배열의 원소로 나눈다. 최대 %(max)d개. 나누는 기준은 **의미가 다른가** 하나뿐이다(규칙 A).
- 원소 하나 안에서 쉼표(,)·세미콜론(;)·가운뎃점(·)·빗금(/)을 뜻을 가르는 구분자로 쓰지 않는다.
- **괄호를 쓰지 않는다.** 괄호 안의 말이 없으면 뜻이 서지 않을 때는 풀어서 한 마디로 잇고,
  그 정도가 아니면 괄호째 버린다.
    '(초밥 등을) 빚다'       -> "초밥을 빚다"
    '(사정에) 어둡다'        -> "사정에 어둡다"
    ...
    '(비유) 주요 상품'       -> "주요 상품"
    '담당(자)'              -> "담당자"
  괄호를 풀었더니 앞 원소와 같은 말이 되면, 그것은 애초에 나눌 것이 아니었다(규칙 A).
- 고유명사는 '고유명사' 임을 밝히거나 원어 표기를 유지한다.

"""

PROMPT = (
    "너는 일본어-한국어 사전 편찬자다. 아래 항목마다 그 낱말의 한국어 뜻을 사전체로 짧게 쓴다.\n"
    "읽는 사람은 일본어를 배우는 한국 사람이다. 뜻은 **그 사람이 읽자마자 알아듣는 한국어**여야 한다.\n\n"
    ...
    + NOTATION_RULES +
    "[출력]\n"
    "{\"results\": [{\"i\": 입력번호, \"sino\": \"한자음\", \"ko\": [\"뜻\", ...], \"why\": \"...\"}]}\n"
    "- sino: w 를 한국 한자음으로 읽은 말. w 에 가나가 섞였거나 한자음으로 읽을 수 없으면 빈 문자열.\n"
    "  **ko 보다 먼저 sino 를 적고**, ko 에 한자음 계열의 말을 쓰려거든 규칙 B 의 네 검사를 거친다.\n"
    "- ko: 규칙 A·B 를 통과한 뜻.\n"
    "- why: 이 요미카타를 같은 표기의 다른 요미카타와 가르는 한 마디. 가를 것이 없으면 빈 문자열.\n\n"
    "[내기 전에] 모든 항목을 한 번 더 훑는다 — 원소가 둘 이상인 항목은 규칙 A 로,\n"
    "한자음 계열의 말을 쓴 항목은 규칙 B 로. 걸리는 것이 있으면 고친 뒤에 낸다.\n"
    "JSON 객체 하나뿐이다. 설명 금지.\n\n"
)
...
# 칸 이름은 ``meanings.DRAFT_SLOT`` 이다.  규약을 고칠 때마다 칸을 새로 판다 —
# 프롬프트만 고치면 캐시가 옛 답을 그대로 내주므로 고침이 데이터에 닿지 못한다.
# 옛 칸은 지우지 않고 남겨 두어 되돌아갈 자리로 쓴다.
cache = Cache(paths.CACHE, DRAFT_SLOT)
...
def ask(group):
    ...
    answer = ask_json(gemini, PROMPT % {"max": MAX_SENSES}
...
REVIEW_WORKERS = 8         # 감수는 따로 조인다 — 1 패스와 부하가 다르다
...
#
# 이 Vertex 프로젝트에 열려 있는 것은 ``gemini-3.8-flash`` 와
...
# 모델 이름은 ``meanings.REVIEW_MODEL`` 에 있다 — 시험도 같은 칸을 읽는다.

REVIEW_PROMPT = (
    "너는 한국어 사전 감수자다. 아래는 일본어 낱말에 **이미 붙어 있는** 한국어 뜻이다.\n"
    "읽는 사람은 일본어를 배우는 한국 사람이다.\n"
    "새로 짓는 자리가 아니다 — 고칠 곳만 고치고, 고칠 곳이 없으면 글자 그대로 되돌린다.\n"
    "그러나 **앞 단계는 아래 최우선 규칙 A·B 를 자주 어겼다.** 통과시키는 것이 기본값이 아니다.\n"
    "항목 하나하나, 원소 하나하나를 의심하며 본다.\n\n"
    "  k   대상 한자        rd  그 한자가 이 낱말에서 갖는 요미카타\n"
    "  w   낱말 표기        ko  지금 붙어 있는 뜻 — 이것을 감수한다\n\n"
    + CORE_RULES +
    "[감수 순서 — 모든 항목을 이 순서대로 본다]\n"
    "1. sino 를 적는다 — w 를 한국 한자음으로 읽은 말. 가나가 섞였거나 읽을 수 없으면 빈 문자열.\n"
    "2. 규칙 A — 원소가 둘 이상이면 **모든 쌍**에 '하나를 지우면 잃는 뜻이 있는가' 를 묻는다.\n"
    "   잃는 뜻이 없으면 대표 하나만 남긴다. 정말 다른 뜻이면 손대지 않는다.\n"
    "3. 규칙 B — 원소 하나하나가 네 검사를 모두 통과하는가. sino 와 같거나 sino 에서 글자만\n"
    "   더하고 뺀 말이면 **특히 엄격하게** 본다. 걸리면 한국 사람이 그 뜻으로 실제로 쓰는 말로 바꾼다.\n"
    "   한자에서 오지 않은 말이라도 어색하면 고쳐 쓴다.\n"
    "4. 괄호가 남았으면 [적는 형식] 대로 풀거나 버린다.\n"
    "5. rd 로 읽었을 때의 뜻인가 — 같은 표기의 다른 요미카타 뜻을 끌어오지 않는다.\n\n"
    + NOTATION_RULES +
    "[출력]\n"
    "{\"results\": [{\"i\": 입력번호, \"sino\": \"한자음\", \"ko\": [\"뜻\", ...], \"fix\": \"...\"}]}\n"
    "- fix: 고쳤으면 어느 규칙(A·B·괄호·요미카타) 때문에 무엇을 고쳤는지 한 마디. 안 고쳤으면 빈 문자열.\n"
    "- 고칠 곳이 없으면 ko 를 **글자 그대로** 되돌린다.\n"
    "**모든 항목에 답한다.** i 는 입력의 i 를 그대로 쓴다.\n"
    "JSON 객체 하나뿐이다. 설명 금지.\n\n"
)

review = Cache(paths.CACHE, REVIEW_SLOT, REVIEW_MODEL)
...
def ask_review(group):
    listing = [{"i": number, "k": items[key]["k"], "rd": items[key]["rd"],
               ...
               for number, key in enumerate(group, 1)]
    answer = ask_json(reviewer, REVIEW_PROMPT % {"max": MAX_SENSES}
                      + json.dumps(listing, ensure_ascii=False))
    rows = answer.get("results", answer if isinstance(answer, list) else [])
    return group, [row for row in rows if isinstance(row, dict)]
...
AGREE_PROMPT = (
    "너는 한국어 사전 감수자다. 아래는 **한 표기의 낱말이 여러 한자 카드에 실린 것**이고,\n"
    ...
    "2. 어느 쪽을 고르든, 고른 답이 **최우선 규칙 A·B 를 통과해야 한다.** 양쪽 모두 걸리면\n"
    "   둘 다 버리고 새로 적는다.\n"
    "       傾倒  '몰두함' / '경도'                -> \"몰두함\"    '경도' 는 한국어에서 다른 뜻이 먼저다(B)\n"
    "       公私  '공적인 일과 사적인 일' / '공사'   -> \"공적인 일과 사적인 일\"\n"
    "       休憩  '휴식' / '휴게'                  -> \"휴식\"\n"
    "   둘 다 통과하면 짧고 흔한 쪽을 쓴다.\n\n"
    + NOTATION_RULES +
    "[출력]\n"
...
agree = Cache(paths.CACHE, AGREE_SLOT, REVIEW_MODEL)
    ...
    for group_index, group in enumerate(batch, 1):
        for key in group:
            ...
            listing.append({"i": number, "g": group_index, "w": items[key]["w"],
                            ...
                            "ko": settled_of(key)})
    answer = ask_json(reviewer, AGREE_PROMPT % {"max": MAX_SENSES}
                      + json.dumps(listing, ensure_ascii=False))
    rows = answer.get("results", answer if isinstance(answer, list) else [])
...
# ---------- Claude 감수 (마지막) ----------
#
# 모델 단계(초안 → pro 감수 → 맞추기)가 끝난 뜻을 Claude Code 세션이 **전건** 다시
# 읽는다.  그 결과는 모델 캐시가 아니라 ``data/claude_review.json`` 에 담긴다.
#
# 감수는 **그 pro 답에 대한** 것이다 — 열쇠에 pro 답의 지문을 싣는다(``claude_key``).
# pro 답이 바뀌면 옛 감수는 붙지 않고, 그 항목은 아래 목록에 다시 올라온다.
claude = load_claude_review(paths.D4_CLAUDE_REVIEW)


def pro_of(key):
    return pro_ko(key, draft_of(key), review, agree)


pending = [key for key in sorted(items)
           if clean((claude.get(claude_key(key, pro_of(key))) or {}).get("ko")) is None]
pending_path = paths.TMP / "s4_claude_pending.json"
dump_json_atomic(pending_path, [
    {"key": key, "k": items[key]["k"], "rd": items[key]["rd"], "w": items[key]["w"],
     "ko": pro_of(key), "claude_key": claude_key(key, pro_of(key))}
    for key in pending], 1)
print(f"[Claude 감수] 감수자 {CLAUDE_REVIEWER} | 대상 {len(items)}"
      f" | 감수 완료 {len(items) - len(pending)} | 남은 것 {len(pending)} -> {pending_path.name}")
if pending:
    print("!! Claude 감수를 못 받은 자리는 pro 단계의 뜻을 쓴다")
